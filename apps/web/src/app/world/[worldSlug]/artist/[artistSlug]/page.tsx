...
/**
 * Public artist identity, resolved inside a world.
 *
 * The world segment is part of the address because a slug is only unique within
 * a world. A single-world deployment can hide the segment in routing later, but
 * the resolution underneath must never guess which world a name belongs to.
 */
export async function generateMetadata({
  params,
}: {
  params: { worldSlug: string; artistSlug: string };
}) {
  const db = await getAppDb();
  const profile = await getPublicArtistProfile(db, params.worldSlug, params.artistSlug, null);
  ...
  return {
    title: `${profile.stageName} — ${profile.worldName}`,
      ...
      <header className="flex flex-col gap-3">
        ...
        {profile.group ? (
          <p className="text-sm text-ink-muted">
            Member of{" "}
            <Link
              href={`/world/${profile.worldSlug}/group/${profile.group.slug}`}
